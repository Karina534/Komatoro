...
public class RequestJwtTokenFilter extends OncePerRequestFilter {
    ...
    private RequestMatcher requestMatcher = PathPatternRequestMatcher
            .withDefaults().matcher(HttpMethod.POST, "/api/users/login");
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (this.requestMatcher.matches(request)){
            if (this.securityContextRepository.containsContext(request)){
                ...
                if (context != null && (context.getAuthentication() instanceof PreAuthenticatedAuthenticationToken authentication)){
